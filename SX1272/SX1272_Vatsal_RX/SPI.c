...
void spiMastInit(void){
	...
	SPCR0 |= (1 << SPE) | (1 << MSTR) | (1 << SPR0) | (1 << SPIE);			//enable SPI,  set as master,  set SCK Fq as Fosc/16, //enable local interrupt
}


void spiMastTrans(unsigned char data){
	SPDR0 = data;
	while(!(SPSR0 & (1 << SPIF)));											// checking last transmission is completed or not
}


unsigned char spiMastRec(void){
	while(!(SPSR0 & (1 << SPIF)));											//Checking if last reception is completed or not
	return SPDR0;
}
